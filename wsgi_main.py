  ...
  padding-bottom: 15px;
            ...
            if my_account.email in local.COMMUNITY_SERVICE and verified_result == 'verified':
                for user_email, hour_count in hour_counts.items():
                    ...
                    t_hours = t_secs / 3600
                    self.wfile.write(f'''<tr><td>{user_email}</td><td>{int(t_secs//3600)}:{int(t_secs//60)%60}:{int(t_secs%60)} --> {math.ceil(t_hours)}</td></tr>'''.encode('utf8'))
            else:
                hour_count = hour_counts[my_account.email]
                t_secs = hour_count.total_seconds()
                t_hours = t_secs / 3600
                self.wfile.write(f'''<tr><td>{my_account.email}</td><td>{int(t_secs//3600)}:{int(t_secs//60)%60}:{int(t_secs%60)} --> {math.ceil(t_hours)}</td></tr>'''.encode('utf8'))
